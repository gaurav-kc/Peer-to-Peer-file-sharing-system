...
void acc_creation_ack(string status)
{
    cout<<status;
    if(status == "success")
    {
...
void sendChunk(string status, string sha, int cno,int socket)
{
    ...
    if(it != master.end())
    {
        ...
        if(it1 != hashtofile.end())
        {
            ...
            int size_of_data2bsent = chunksize+15;
            char buffer[size_of_data2bsent];
            int val = fread(buffer,1,chunksize,fptr);
            string message = to_string(val) + ":" + buffer;
